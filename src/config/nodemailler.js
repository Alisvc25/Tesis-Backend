...
// Soporta dos “familias” de variables: MAIL_* y *_MAILTRAP
const host =
    getEnv("MAIL_HOST") ||
    getEnv("HOST_MAILTRAP") ||
    "smtp.gmail.com";

const port = Number(
    getEnv("MAIL_PORT") ||
    getEnv("PORT_MAILTRAP") ||
    587
);

// Para Gmail: 465 = SSL (secure true) | 587 = STARTTLS (secure false)
const secure = String(getEnv("MAIL_SECURE") || "").toLowerCase() === "true"
    ? true
    : port === 465;

const user =
    getEnv("MAIL_USER") ||
    getEnv("USER_MAILTRAP");

const pass =
    getEnv("MAIL_PASS") ||
    getEnv("PASS_MAILTRAP");

if (!user || !pass) {
    console.warn("⚠️ Faltan credenciales de correo. Revisa MAIL_USER/MAIL_PASS o USER_MAILTRAP/PASS_MAILTRAP.");
}
...
const transporter = nodemailer.createTransport({
    ...
    secure,
    auth: { user, pass },
    // Esto ayuda en algunos hosts cuando usan 587
    requireTLS: port === 587,
});
...
async function verifyMail() {
    try {
        await transporter.verify();
        console.log("✅ SMTP listo:", { host, port, secure, user });
    } catch (e) {
        console.error("❌ SMTP error:", e.message);
    }
}

module.exports = { transporter, verifyMail };
